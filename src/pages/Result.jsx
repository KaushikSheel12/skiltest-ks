 ...
 mt-7 flex-wrap  px-3 r font-semibold  "
          >
            ...
            {header.map(({ icon, id, title }) => (
              <p
                key={id}
                className="font-semibold text-gray-600  text-[14px] text-center py-3  w-full grid place-items-center"
              >
                <span>{icon}</span>
                {title}
              </p>
      ...
      <button className="flex md:hidden justify-center mx-auto w-[90%] h-9 px-4 py-2 mb-7  mt-2 rounded-md bg-yellow-400 hover:bg-yellow-300">
        Reattempt Test
      </button>
    </>
  );
